...
## Fork: merging upstream & adding strings

This fork does not use Bluesky's Crowdin, so a few things differ:

- **Compiled catalogs (`messages.ts`) are gitignored** and regenerated by
  `intl:compile` (via `postinstall`), so they never cause merge conflicts.
- **Only `en/messages.po` is fork-touched.** The other locales come straight
  from upstream and fast-forward on merge; `en/messages.po` is the one catalog
  that conflicts when both we and upstream change it.
- **Never hand-merge a catalog conflict** - they are generated. Run:

  ```bash
  pnpm intl:resync
  ```

  It takes upstream's version of any conflicted catalog (keeping their
  translations), re-extracts from our merged source so fork strings are
  re-added, recompiles, and stages the result. Review, then commit the merge.

- **After adding new strings**, run `pnpm intl:extract && pnpm intl:compile`
  (or `pnpm intl:build`) so they land in `en/messages.po` and the runtime
  catalog - otherwise the string renders as its message id, not the text.
- Fork-added strings stay in English (the source) in other locales until we
  translate them.
