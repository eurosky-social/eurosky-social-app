// @ts-check
/**
 * Regenerate static web/embed identity from src/config/brand.json: pre-boot
 * colours and logos, HTML metadata, Go-template constants, and the standalone
 * embed snippet endpoint.
 *
 * The pre-boot files render before any JS / per-user accent preference loads
 * (and base.html is served by the Go server, not the Expo web build), so they
 * use the org's shipped default accent.
 *
...
const SOCIAL_CARD = `https://${BRAND_HOST}/static/social-card-default-gradient.png`
const PUBLIC_API = brand.services.publicApi
const EMBED_SERVICE = brand.services.embed
...
const PREBOOT_TARGETS = ['web/index.html', 'bskyweb/templates/base.html']
const EMBED_HTML_TARGETS = ['bskyembed/index.html', 'bskyembed/post.html']
const GENERATED_GO = 'bskyweb/cmd/bskyweb/brand_generated.go'
const GENERATED_EMBED_SNIPPET = 'bskyembed/snippet/brand.generated.ts'
...
/** Static identity used by the standalone bskyembed HTML entry points. */
function embedMetaBlock(i) {
  return [
    `${i}<title>${BRAND_NAME} Embed</title>`,
    `${i}<link rel="preconnect" href="${PUBLIC_API}">`,
    `${i}<link rel="apple-touch-icon" sizes="180x180" href="/static/apple-touch-icon.png">`,
    `${i}<link rel="icon" type="image/png" sizes="32x32" href="/static/favicon-32x32.png">`,
    `${i}<link rel="icon" type="image/png" sizes="16x16" href="/static/favicon-16x16.png">`,
    `${i}<link rel="mask-icon" href="/static/safari-pinned-tab.svg" color="${A.primary_500}">`,
    `${i}<meta name="theme-color" content="${A.primary_500}">`,
    `${i}<meta name="application-name" content="${BRAND_NAME}">`,
    `${i}<meta name="generator" content="bskyweb">`,
  ].join('\n')
}

/** Generated constants exposed to the Go template renderer. */
function generatedGo() {
  return `// Code generated by scripts/sync-brand-web.mjs. DO NOT EDIT.\n\npackage main\n\nconst brandName = ${JSON.stringify(BRAND_NAME)}\nconst brandHost = ${JSON.stringify(BRAND_HOST)}\nconst brandHomeURL = ${JSON.stringify(`https://${BRAND_HOST}`)}\nconst brandSocialCardURL = ${JSON.stringify(SOCIAL_CARD)}\nconst brandEmbedService = ${JSON.stringify(EMBED_SERVICE)}\n`
}

/** Quote a value using the repository's preferred TypeScript string style. */
function quoteTypeScript(value) {
  return `'${value.replace(/\\/g, '\\\\').replace(/'/g, "\\'")}'`
}

/** Generated default endpoint used by the standalone embed snippet. */
function generatedEmbedSnippet() {
  return `// Code generated by scripts/sync-brand-web.mjs. DO NOT EDIT.\n\nconst DEFAULT_EMBED_URL = ${quoteTypeScript(EMBED_SERVICE)}\n`
}
...
/** @param {string} content */
function renderPrebootFile(content) {
  let out = content
...
/** @param {string} rel @param {string} next */
function syncTextFile(rel, next) {
...
for (const rel of PREBOOT_TARGETS) {
  const file = path.join(ROOT, rel)
  syncTextFile(rel, renderPrebootFile(fs.readFileSync(file, 'utf8')))
}

for (const rel of EMBED_HTML_TARGETS) {
  const file = path.join(ROOT, rel)
  const orig = fs.readFileSync(file, 'utf8')
  syncTextFile(rel, regenRegion(orig, 'embed-meta', 'html', embedMetaBlock))
}

syncTextFile(GENERATED_GO, generatedGo())
syncTextFile(GENERATED_EMBED_SNIPPET, generatedEmbedSnippet())
