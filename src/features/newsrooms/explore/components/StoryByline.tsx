...
import {type ExploreArticle} from '../cluster'
...
export function PublisherLabel({
  ...
  const t = useTheme()
  const {t: l} = useLingui()
  const name = getPublisherName(profile) || article.publisher.id

  return (
    <Link
      to={`/newsroom/${article.publisher.did}`}
        ...
        {articles.slice(0, 4).map((article, index) => (
          <View
            ...
            <UserAvatar
              type="user"
              size={18}
              avatar={profiles.get(article.publisher.did)?.avatar}
            />
          </View>
        ))}
      </View>
