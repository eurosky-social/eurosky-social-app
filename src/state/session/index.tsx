...
import {
  ...
  oauthSessionToSessionAccountOrThrow,
...
const BundleContext = createContext<
  AuthSessionBundle | PublicSessionBundle | null
>(null)
...
export function Provider({children}: React.PropsWithChildren<{}>) {
  ...
  const login = useCallback<SessionApiContext['login']>(
    async (params, logContext) => {
      ...
      const {bundle, account} = params.oauthSession
        ? await createOAuthSessionBundle(params.oauthSession)
  ...
  const resumeSession = useCallback<SessionApiContext['resumeSession']>(
    async (storedAccount, isSwitchingAccounts = false) => {
      ...
      const {bundle, account} = storedAccount.isOauthSession
        ? await resumeOAuthSessionBundle(storedAccount)
  ...
  >(async () => {
    const bundle = store.getState().currentBundleState.bundle as unknown as
      AuthSessionBundle | PublicSessionBundle
    if (!bundle.session) return undefined // logged out: nothing to refresh

    if (!isPasswordSessionBundle(bundle)) {
      await bundle.session.getTokenInfo(true)
      const account = await oauthSessionToSessionAccountOrThrow(
        bundle.pdsClient,
        bundle.session,
      )
      if (store.getState().currentBundleState.bundle !== bundle) {
     ...
     * against an account that is no longer active.
     */
    if (store.getState().currentBundleState.bundle !== bundle) {
      throw new Error('The session changed while it was being refreshed')
    }
  ...
  useEffect(() => {
    return persisted.onUpdate('session', nextSession => {
       ...
       * considers current. OAuth credentials live outside this snapshot, so an
       * OAuth account entry is also a resumable session.
       */
      const hasSyncedSession = Boolean(
        syncedAccount &&
        (syncedAccount.isOauthSession || syncedAccount.refreshJwt),
      )
      const syncedDid = hasSyncedSession ? syncedAccount?.did : undefined
      if (
        syncedDid === undefined &&
      ...
      if (syncedAccount && hasSyncedSession) {
        if (syncedAccount.did !== state.currentBundleState.did) {
          void resumeSession(syncedAccount)
        } else if (!syncedAccount.isOauthSession && syncedAccount.refreshJwt) {
          /*
           ...
           */
          const prevBundle = state.currentBundleState
            .bundle as unknown as SessionBundle
  ...
  useEffect(() => {
    if (currentBundleRef.current !== bundle) {
      ...
      // Replaced password bundles must never consume another refresh token.
